...
def str2list(x):
    ...
    if x != flatten_to_values(x):
        print(x, flatten_to_values(x))
...
def run_score(val_file, pred_file, save_dir, exclude_keys=""):
    ...
    df.loc[len(df)] = [
        "综合",
        len(all_imgnames),
        sum(all_mfh) / len(all_mfh),
        sum(all_mh) / len(all_mh),
    ]
    df.to_excel(os.path.join(save_dir, "score.xlsx"), index=False)


if __name__ == "__main__":
    val_file = "/home/xiaozheng/llm_uie_sft/qa/short_doc_doc_8000_one_shot_n_ratio0/val.json"
    pred_file = "/home/xiaozheng/llm_uie_sft/qa/short_doc_doc_8000_one_shot_n_ratio0/gpt4_generated_pred_oneshot.jsonl"
    save_dir = "/home/xiaozheng/llm_uie_sft/qa/short_doc_doc_8000_one_shot_n_ratio0/gpt4_oneshot_end2end_score"
